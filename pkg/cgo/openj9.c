...
static int write_command_openj9(int fd, const char* cmd) {
...
static int read_response_openj9(int fd, const char* cmd, int print_output) {
    size_t size = 8192;
...
static void detach(int fd) {
    if (write_command_openj9(fd, "ATTACH_DETACHED") != 0) {
        return;
    }
...
int jattach_openj9(int pid, int nspid, int argc, char** argv, int print_output) {
    ...
    if (write_command_openj9(fd, cmd) != 0) {
        perror("Error writing to socket");
        close(fd);
        return 1;
    }

    int result = read_response_openj9(fd, cmd, print_output);
    if (result != 1) {
        detach(fd);
    }
    close(fd);
